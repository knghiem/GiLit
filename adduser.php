...
<head>
	...
	<style>
        ...
        input,select {margin: 0px 0px 0px 30px}
...
<body style="text-align:center; margin:75px;" ng-controller="RegistrationController as registration">
	...
	<div>
		<?php
			...
			if( !empty($_POST["username"]) ){
                ...
                if (!$error){
									  $com_id_str = mysqli_real_escape_string($db_conn, $_POST['com_id']);
				    ...
				    $cmd = "INSERT INTO users (name, email, username, password, com_id) VALUES ('"
				                . $name . "','" . $email . "','" . $user . "','" . $pass . "','" . $com_id . "');";
	...
	<form action="adduser.php" method="post" name="registrationForm" novalidate
          ng-submit="registration.submit(registrationForm.$valid)">
					<span class="glyphicon glyphicon-home"></span><select name="com_id">
					<?php
					//Get all the current schools/communities to populate the drop-down menu
					$db_conn = mysqli_connect("localhost", "root", "");
					if (!$db_conn)
						die("Unable to connect: " . mysqli_connect_error());  // die is similar to exit
					...
					if ($num_row > 0){
								while($row = mysqli_fetch_array($result)){
											//echo the name of the campus as an option
